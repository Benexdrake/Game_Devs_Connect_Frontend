...
describe.each([{test:'backend', frontend:false, id:999, ownerId:'0'}, {test:'frontend', frontend:true, id:888, ownerId:'0'}])
("File Service: $test", ({id, ownerId, frontend}) =>
{
    ...
    {
        const response = await getFileById(id+'', frontend);
    ...
    {
        const response = await deleteFile(id+'', frontend);
